...
// --- 1. RAFT FSM & STORE (Inalterado) ---
// [As definições de command, Store, NewStore, Apply, Snapshot, Restore, e FSMSnapshot permanecem as mesmas do seu arquivo]
// ... (Omesmo código das linhas 30-142 do seu main.go original) ...
...
// --- 2. HANDLERS E ROUTER (Inalterado) ---
// [As definições de statusHandler, joinHandler, leaveHandler, e SetupRouter permanecem as mesmas]
// ... (Omesmo código das linhas 145-231 do seu main.go original) ...
...
func (s *Store) joinHandler(c *gin.Context) {
	...
	// A porta 7001 (RaftAddr) é usada here, o que está correto.
...
// --- 3. FUNÇÕES DE ORQUESTRAÇÃO (CORRIGIDAS) ---

// CORREÇÃO 2: Função para Health Check TCP (Inalterada)
...
// **** ALTERAÇÃO 1: Assinatura e lógica do setupRaft corrigidas ****
func setupRaft(id string, port int, advertiseAddr string, fsm *Store, bootstrap bool) (*raft.Raft, error) {
	...
	// *** CORREÇÃO: Remove IP Fixo ('172.17.0.1') e usa o 'advertiseAddr' do parâmetro ***
	...
	// *** CORREÇÃO: Retorno alterado (não precisa mais retornar o endereço) ***
	return r, nil
}

// CORREÇÃO 2: Monitoramento de Peers com Health Check Duplo (Inalterado)
// ... (Omesmo código das linhas 290-332 do seu main.go original) ...
...
// CORREÇÃO 4: Observer de Mudanças de Liderança (Inalterado)
// ... (Omesmo código das linhas 335-353 do seu main.go original) ...
...
// **** ALTERAÇÃO 2: Assinatura e lógica do joinCluster corrigidas ****
func joinCluster(id string, localRaftAddr string, leaderHTTPPort int, peersStr string) {
	...
	for _, peer := range peers {
		...
		// *** CORREÇÃO: Remove IP Fixo ('0.0.0.0') e usa 'localRaftAddr' do parâmetro ***
		...
		// NOTA: Fechar o body é importante, mas o 'defer' dentro de um loop só executará no final da função.
		// Melhor fechar manualmente.
		
		if resp.StatusCode == http.StatusOK {
			log.Printf("Nó %s se juntou ao cluster via %s com sucesso!", id, peer)
			resp.Body.Close() // Fecha o corpo imediatamente
			return
		}
...
func main() {
	var (
		...
		bootstrap bool
		peersStr  string
		raftAddr  string // **** ALTERAÇÃO 3: Nova flag adicionada ****
	)
	...
	// **** ALTERAÇÃO 3: Nova flag definida ****
	flag.StringVar(&raftAddr, "raft-addr", "", "Endereço Raft anunciável (ex: raft-leader:7000)")
	flag.Parse()

	if nodeID == "" {
		log.Fatal("O ID do nó (--id) deve ser fornecido.")
	}
	// **** ALTERAÇÃO 3: Validação da nova flag ****
	if raftAddr == "" {
		log.Fatal("O endereço Raft (--raft-addr) deve ser fornecido (ex: --raft-addr=127.0.0.1:7000).")
	}
	...
	// 3. Configura o Raft
	// **** ALTERAÇÃO 4: Passa 'raftAddr' para setupRaft e ajusta o retorno ****
	raftLog, err = setupRaft(nodeID, raftPort, raftAddr, store, bootstrap)
	if err != nil {
	...
	store.NodeID = nodeID
	store.RaftAddr = raftAddr // Usa o raftAddr da flag
	...
	if !bootstrap && peersStr != "" {
		...
		// **** ALTERAÇÃO 5: Passa 'raftAddr' para joinCluster ****
	...
	// 7. Tratamento de Shutdown (Inalterado)
